...
function clickBead(event) {
  ...
  setActiveBead(objectUuid, rosayItem, true);
}
...
async function selectNextBead() {
  ...
  setActiveBead(objectUuid, rosaryItem, true);
...
async function selectPreviousBead() {
  ...
  console.log("Rosary Item: ", rosaryItem);
...
async function setActiveBead(objectUuid, rosaryItem, moveBead = false) {
  console.log("Move Bead: ", moveBead);
  ...
  if (!isCross && objectUuid.parent.type == "Scene") {
    ...
    if (rosaryItem.description == "HailHolyQueen") {
      ...
      synth.triggerAttackRelease("E4", "8n", now + delay);
      synth.triggerAttackRelease("G4", "8n", now + delay * 2);
    }
